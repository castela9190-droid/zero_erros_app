...
def gerar_relatorio_pro(dados, inputs, user, foto_bytes=None):
    ...
    pdf.set_font("Helvetica", "I", 10); pdf.cell(0, 10, f"Perito Avaliador: {user} | Data: {datetime.now().strftime('%d/%m/%Y')}", new_x="LMARGIN", new_y="NEXT", align="C"); pdf.ln(5)
    
    # 1. Identificação
    pdf.set_font("Helvetica", "B", 12); pdf.set_fill_color(230, 230, 230)
    pdf.cell(0, 10, "1. Identificacao do Imovel", new_x="LMARGIN", new_y="NEXT", fill=True); pdf.ln(2)
    ...
    pdf.cell(0, 6, f"Estado: {inputs['estado']} | Norma: {inputs['norma']}", new_x="LMARGIN", new_y="NEXT"); pdf.ln(5)

    # FOTO
    if foto_bytes:
        try:
            with open("temp_img.jpg", "wb") as f: f.write(foto_bytes.getbuffer())
            pdf.image("temp_img.jpg", x=120, y=45, w=70)
        except: pass
    
    # 2. Dados
    pdf.set_y(90)
    pdf.set_font("Helvetica", "B", 12); pdf.cell(0, 10, "2. Analise Tecnica e Areas", new_x="LMARGIN", new_y="NEXT", fill=True); pdf.ln(2)
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(0, 6, f"Area Bruta Privativa (Registo): {inputs['abp']} m2", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 6, f"Area Util Medida (Laser): {inputs['au']} m2", new_x="LMARGIN", new_y="NEXT"); pdf.ln(2)
    pdf.cell(0, 6, f"Preco Base: {inputs['preco_base']:,.2f} EUR/m2 | Fator Global: {dados['fator_global']:.2f}", new_x="LMARGIN", new_y="NEXT"); pdf.ln(5)

    # 3. Valor
    pdf.set_font("Helvetica", "B", 12); pdf.cell(0, 10, "3. Valor de Mercado", new_x="LMARGIN", new_y="NEXT", fill=True)
    ...
    plt.text(0.5, 0.5, texto, fontsize=14, ha='center', va='center'); plt.axis('off')
    
    img_buffer = io.BytesIO()
    plt.savefig(img_buffer, format='png', bbox_inches='tight', dpi=150); img_buffer.seek(0)
...
if not st.session_state.logged_in:
    ...
    with tab1:
        ...
        with col_dados:
            ...
            # --- CORREÇÃO DO ERRO DE GEOCODIFICAÇÃO AQUI ---
            busca_morada = st.text_input("🔍 Pesquisar Morada (Se der erro, use coordenadas)", help="Ex: Avenida da Liberdade, Lisboa")
            
            lat_inicial, long_inicial = 38.736946, -9.142685 
            
            if busca_morada:
                try:
                    # Definimos um User-Agent único e um timeout maior (10 segundos)
                    geolocator = Nominatim(user_agent="zero_erros_app_v2_architect_solver")
                    location = geolocator.geocode(busca_morada, timeout=10)
                    ...
                    # O "Try/Except" apanha o erro e mostra um aviso amigável em vez de crashar
                    st.warning("⚠️ O serviço de mapas automático está temporariamente indisponível (bloqueio de segurança da Cloud). Por favor insira a Latitude/Longitude manualmente abaixo.")
                except Exception as e:
                    st.error(f"Erro inesperado no mapa: {e}")
            ...
            st.divider()
            st.subheader("2. Caracterização")
            
            c_art, c_tip = st.columns(2)
            artigo = c_art.text_input("Artigo", "U-12345")
            ...
            st.divider()
            st.subheader("3. Homogeneização")
            
            preco_base = st.number_input("💶 Preço Base (€/m2)", value=3500.0, step=50.0)
            
            col_c1, col_c2, col_c3 = st.columns(3)
            c_loc = col_c1.slider("Localização", 0.8, 1.2, 1.0, 0.05)
            c_qual = col_c2.slider("Qualidade", 0.8, 1.2, 1.0, 0.05)
            
            mapa_estado = {"Novo": 1.0, "Usado (Bom)": 0.9, "Usado (Médio)": 0.8, "Para Obras": 0.6, "Ruína": 0.4}
            val_est = mapa_estado.get(estado, 0.8)
            c_est = col_c3.slider("Estado", 0.3, 1.0, val_est, 0.05)
            ...
            if st.button("🚀 Calcular Valor PRO", type="primary"):
                ...
                if erro: st.error(erro)
                    ...
                    registo = {"Data": datetime.now().strftime("%H:%M"), "Artigo": artigo, "Valor": res['valor_final']}
                    st.session_state.historico_local.append(registo)
                    st.success("✅ Avaliação Calculada!")
        ...
        with col_mapa:
            ...
            if "resultado_pro" in st.session_state:
                ...
                pdf_bytes = gerar_relatorio_pro(res, st.session_state.inputs_pro, st.session_state.username, st.session_state.foto_atual)
                st.download_button("📄 Baixar Relatório PRO (Com Foto)", data=pdf_bytes, file_name=f"Avaliacao_{artigo}.pdf", mime="application/pdf", type="primary")
